...
def my_app(cfg: DictConfig) -> None:
    ...
    if cfg.model.type == "p-tuning" or cfg.model.type == "ensemble":
        ...
        for relation_id in train_relation_ids:
        ...
        with open_dict(cfg):
            ...
            cfg.model.relclf = OmegaConf.create({
              "model": OmegaConf.to_container(relclf_model_cfg, resolve=False),
              "data": OmegaConf.to_container(relclf_data_cfg, resolve=False)
              })
        ...
        trainer = Trainer(cfg)
        trainer.train()
    elif cfg.model.type == "oracle":  # oracle
        # we have to load the p-tuning config and the relclf configs
        # update the id to include the relclf
        p_tuning_cfg = OmegaConf.load(cfg.model.ptuning.model_path)
        with open_dict(cfg):
